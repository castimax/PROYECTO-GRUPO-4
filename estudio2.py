...
for name, model in models.items():
    ...
    plt.show()
     ...
     # Visualizar y guardar la matriz de confusión con seaborn
    plt.figure(figsize=(10, 7))
    sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="Blues", xticklabels=True, yticklabels=True)
    plt.title(f"Matriz de Confusión para {name}")
    plt.ylabel('Verdaderos')
    plt.xlabel('Predicciones')
    plt.savefig(f'{name.replace(" ", "_")}_confusion_matrix.png')  # Guardar con un nombre único
    plt.close()  # Cerrar la figura actual para evitar superposiciones